...
const _grecaptchaMock: ReCaptcha = {
  execute: (siteKey, options) => {
    return Promise.resolve(`siteKey: ${siteKey}, options: ${JSON.stringify(options)}`);
  },
  ready: (callback) => {
    callback();
  },
...
describe("useGoogleReCaptcha", () => {
  ...
  test("execute", () => {
    const { result } = renderHook(() => useGoogleReCaptcha("SITE_KEY"));
    expect(result.current.executeGoogleReCaptcha).toMatchInlineSnapshot("[Function]");
  });

  test("execute", () => {
    const { result } = renderHook(() => useGoogleReCaptcha("SITE_KEY"));
    expect(result.current.executeGoogleReCaptcha("action")).toMatchInlineSnapshot("Promise {}");
  });

  test("execute enterprise", () => {
    const { result } = renderHook(() => useGoogleReCaptcha("SITE_KEY", { enterprise: true }));
    expect(result.current.executeGoogleReCaptcha("action")).toMatchInlineSnapshot("Promise {}");
  });
